...
const ExamSubmissions: React.FC = () => {
  ...
  useEffect(() => {
    if (courseId) {
      axios.get(`http://localhost:5001/api/exams/course/${courseId}`,{
        headers:{
      ...
      axios.get(`http://localhost:5001/api/exams/submissions/${courseId}`,{
        headers:{
          "Authorization":`Bearer ${localStorage.getItem('token')}`
        }
      })
  ...
  const handleSubmitMarking = () => {
    ...
    axios.post(`http://localhost:5001/api/submissions/marking`, markingData,{
      headers:{
        "Authorization":`Bearer ${localStorage.getItem('token')}`
      }
    })
  ...
  return (
    <div>
      <div className="m mx-auto p-6 bg-white rounded-lg shadow-md">
        ...
        <h3 className="text-xl mb-4">Submissions for Course</h3>
