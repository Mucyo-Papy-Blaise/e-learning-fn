...
export const MenuBar = ({ editor, onUploadError }: MenuBarProps) => {
  ...
  const addTable = useCallback((e?: React.MouseEvent) => {
    e?.preventDefault();
    e?.stopPropagation();
  ...
  const toggleOrderedList = useCallback((e?: React.MouseEvent) => {
    e?.preventDefault();
    e?.stopPropagation();
    editor?.chain().focus().toggleOrderedList().run();
  }, [editor]);

  const toggleTaskList = useCallback((e?: React.MouseEvent) => {
    e?.preventDefault();
    e?.stopPropagation();
    ...
    <>
      ...
      >
        ...
        <div className="flex items-center gap-1">
          ...
          <ToolbarButton icon={Code} isActive={editor.isActive("code")} onClick={() => editor.chain().focus().toggleCode().run()} title="Code" />
        </div>
        ...
        {/* Lists */}
        <div className="flex items-center gap-1">
          <ToolbarButton icon={List} isActive={editor.isActive("bulletList")} onClick={toggleBulletList} title="Bullet List" />
          <ToolbarButton icon={ListOrdered} isActive={editor.isActive("orderedList")} onClick={toggleOrderedList} title="Numbered List" />
          <ToolbarButton icon={CheckSquare} isActive={editor.isActive("taskList")} onClick={toggleTaskList} title="Task List" />
        </div>
        ...
        <div className="flex items-center gap-1">
          ...
          <ToolbarButton icon={AlignJustify} isActive={editor.isActive({ textAlign: "justify" })} onClick={() => editor.chain().focus().setTextAlign("justify").run()} title="Justify" />
        </div>
        ...
        <div className="flex items-center gap-1">
          ...
          <ToolbarButton icon={TableIcon} onClick={addTable} title="Add Table" />
        </div>
        ...
        {/* Other Actions */}
        <div className="flex items-center gap-1">
          <ToolbarButton icon={Quote} isActive={editor.isActive("blockquote")} onClick={() => editor.chain().focus().toggleBlockquote().run()} title="Quote" />
          <ToolbarButton icon={Minus} onClick={() => editor.chain().focus().setHorizontalRule().run()} title="Horizontal Rule" />
          <ToolbarButton icon={RemoveFormatting} onClick={() => editor.chain().focus().unsetAllMarks().run()} title="Remove Formatting" />
        </div>
        ...
        {/* Undo/Redo */}
        <div className="flex items-center gap-1">
          <ToolbarButton icon={Undo} onClick={() => editor.chain().focus().undo().run()} disabled={!editor.can().chain().focus().undo().run()} title="Undo" />
          <ToolbarButton icon={Redo} onClick={() => editor.chain().focus().redo().run()} disabled={!editor.can().chain().focus().redo().run()} title="Redo" />
        </div>
      </div>
    </>
  );
